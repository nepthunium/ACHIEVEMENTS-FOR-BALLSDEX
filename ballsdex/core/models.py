...
class BallInstance(models.Model):
    ...
    def long_description(
        ...
        *,
        ...
        base_health = self.countryball.health
        base_attack = self.countryball.attack

        if not short:
            # Calculate the final health and attack stats with bonuses
            final_health = base_health + (base_health * self.health_bonus / 100)
            final_attack = base_attack + (base_attack * self.attack_bonus / 100)
            
            text += f" HP:{final_health:.0f} ATK:{final_attack:.0f}"
